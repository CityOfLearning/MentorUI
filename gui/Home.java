...
import com.dyn.mentor.gui.GiveAchievement;
...
public class Home extends Show {
	...
	private void switchMode() {
		for (String student : MentorUI.roster) {
			PacketDispatcher.sendToServer(new MentorCommandMessage("/gamemode " + (areStudentsInCreative ? "0 " : "1 ") + student.split("-")[0]));
		}
	...
	public void setup() {
		...
		registerComponent(
				new TextLabel(width / 3, (int) (height * .1), width / 3, 20, "Home", TextAlignment.CENTER));
		...
		rosterDisplayList = new ScrollableDisplayList((int) (width * .15), (int) (height * .45), (int)(width / 3.3), 105, 15,
				rlist);
		rosterDisplayList.setId("roster");
		registerComponent(rosterDisplayList);
		
		//TODO Change Total to actual total count of roster
		numberOfStudentsOnRoster = new TextLabel((int)(width * .37), (int)(height * .4), 90, 20, MentorUI.roster.size() + "/" + "Total", TextAlignment.LEFT);
		registerComponent(numberOfStudentsOnRoster);
		
		registerComponent(new Button((int)(width * .15), (int) (height * .35), 20, 20, "<>")
		.addHoverText("Refresh").doesDrawHoverText(true).setClickListener(but -> updateUserList()));
		
		//Manage Students
		registerComponent(new Button((int) (width * .55), (int) (height * .72), (int)(width / 3.3), 20, "Teleport Students to me")
		.setClickListener(but -> teleportStudentsToMe()));
		
		registerComponent(new Button((int) (width * .55), (int) (height * .82), (int)(width / 3.3), 20, "Remove Effects")
		...
		freezeButton.setIsEnabled(true).addHoverText(freezeText).doesDrawHoverText(true).setClickListener(but -> freezeUnfreezeStudents());
		...
		muteButton.setIsEnabled(true).addHoverText(muteText).doesDrawHoverText(true).setClickListener(but -> muteUnmuteStudents());
		...
		// The background
		Picture background = new Picture((int)(width/8), (int) (height * .15), (int) (width * (6.0 / 8.0)), (int) (height * .8),
				new ResourceLocation("dyn", "textures/gui/background.png"));
		
		registerComponent(new Shape((int)(width/7.4), (int)(height*.23) + 25, (int)(width * .733), 1, ShapeType.RECT, Color.GRAY));

...
public class ManageStudent extends Show {
	...
	public ManageStudent() {
		...
		title = "Teacher GUI Manage A Student";
	...
	private void freezeUnfreezeStudent() {
		if (selectedEntry != null) {
			if(isFrozen)
				PacketDispatcher.sendToServer(new MentorCommandMessage("/p user " + selectedEntry.getTitle().split("-")[0] + " group add _FROZEN_"));	
			else 		
				PacketDispatcher.sendToServer(new MentorCommandMessage("/p user " + selectedEntry.getTitle().split("-")[0] + " group remove _FROZEN_"));
			
			PacketDispatcher.sendToServer(new RequestFreezePlayerMessage(selectedEntry.getTitle().split("-")[0], isFrozen));
	...
	private void switchMode() {
		if (selectedEntry != null) {
			PacketDispatcher.sendToServer(new MentorCommandMessage("/gamemode " + (isStudentInCreative ? "0 " : "1 ") + selectedEntry.getTitle().split("-")[0]));
	...
	public void setup() {
		...
		registerComponent(new TextLabel(width / 3, (int) (height * .1), width / 3, 20, "Manage a Student",
		...
		// GUI main section
		registerComponent(new Button((int)(width * .15), (int) (height * .25), 20, 20, "<>")
		...
		freezeButton.setIsEnabled(true).addHoverText(freezeText).doesDrawHoverText(true).setClickListener(but -> freezeUnfreezeStudent());
		...
		muteButton.setIsEnabled(true).addHoverText(muteText).doesDrawHoverText(true).setClickListener(but -> muteUnmuteStudent());
		...
		modeButton.setIsEnabled(true).addHoverText(modeText).doesDrawHoverText(true).setClickListener(but -> switchMode());
		...
		registerComponent(new PictureButton((int) (width * .7), (int) (height * .25), 50, 25,
				new ResourceLocation("minecraft", "textures/items/melon.png")).setIsEnabled(true)
						.addHoverText("Heal Students").doesDrawHoverText(true)
		...
		registerComponent(new PictureButton((int) (width * .7), (int) (height * .365), 50, 25,
				new ResourceLocation("minecraft", "textures/items/chicken_cooked.png")).setIsEnabled(true)
						.addHoverText("Feed Students").doesDrawHoverText(true)
						.setClickListener(but -> feedStudent()));
		
		registerComponent(new Button((int) (width * .55), (int) (height * .6), (int)(width / 3.3), 20, "Teleport to Student")
		.setClickListener(but -> teleportToStudent()));

		registerComponent(new Button((int) (width * .55), (int) (height * .7), (int)(width / 3.3), 20, "Teleport Student to Me")
				.setClickListener(but -> teleportStudentTo()));
		
		registerComponent(new Button((int) (width * .55), (int) (height * .8), (int)(width / 3.3), 20, "Remove Effects")
		...
		dynUsernameLabel = new TextLabel((int)(width * .15), (int) (height * .8), (int)(width/2.5), 20, "Username: " + dynUsername);
		dynPasswordLabel = new TextLabel((int)(width * .15), (int) (height * .85), (int)(width/2.5), 20, "Password: " + dynPassword);
